...
Preview workload builds publish to retained repositories under
`mem9-on-aws/preview/*`. Production builds remain under
`mem9-on-aws/*`. The preview role can pull both namespaces for IaC-only preview
reuse but can push only the preview path; the production role cannot push the
preview path. SST receives the selected namespace explicitly for both preview
deploy passes.
...
4. Merge the role/workflow-only change; changed-path classification must skip
   application mutation for that merge.
5. Pause deployment workflows before enabling preview repository writes.
6. Create and read back all four retained preview repositories.
7. Update and read back preview/production ECR write denies, then custom-simulate
   both directions before publishing application code.
8. Unpause and open the application rollout PR. Verify its disposable preview,
   rerun/resume behavior, and cleanup using only the preview role/repositories.
9. Verify the production approval wait, then approve the intentional workload
   image build and production SST deployment.
10. Verify representative opposite-stage requests and CloudTrail role use.
11. Redeploy the owner stack with the legacy trust disabled and remove the old
   repository secret.

Rollback before step 11 restores the old repository secret/workflow reference.
After step 11, rollback uses the explicit `--enable-legacy` operator action.
