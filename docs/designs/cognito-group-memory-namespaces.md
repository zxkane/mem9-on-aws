...
accepts both stable `a` and `b` slot keys for each allowlisted issuer. Namespace-aware
...
ECS injects the transport key from the stage-scoped
`/mem9-on-aws/<stage>/namespace/transport-signing-keys` SSM SecureString. The
server task execution role, not the task role, performs that fetch before
container startup. This reuses the existing application-region project SSM KMS
context in the workload permissions boundary and avoids an account-global
boundary change for the namespace feature.

The keyring uses stable `a` and `b` random-password slots, publishes both keys,
and separately records the active signing slot. Every transport envelope carries
the stable slot ID in `kid`. Because ECS injects SSM values only when a
container starts, the task definition also carries a non-secret SHA-256 keyring
revision so any slot or active-key change creates a new revision and rolling
deployment. Rotation is always two deployments: increment only the inactive
slot revision and deploy; after that rollout is healthy, switch
`MEM9_TRANSPORT_SIGNING_ACTIVE_SLOT` and deploy again. The first deployment
teaches every new task the future signing key while Gateway still signs with
the old key; the second starts signing with that already accepted key. Changing
both slot revisions, or changing a slot revision and active slot in one
deployment, is prohibited. Stable `kid` values ensure old and new tasks interpret
each signature against the same key during both mixed rolling windows. This
follows the
[ECS Parameter Store injection contract](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/secrets-envvar-ssm-paramstore.html),
which does not refresh an updated parameter inside a running container.

For `pr-N` stages, `preview-alpha` and `preview-beta` are authoritative fixture
slots. Cognito may replace an autogenerated M2M client during a rerun, changing
its client ID while the shared preview database survives. Bootstrap therefore
revokes memberships for obsolete fixture principals, disables those principals,
deletes their stale client bindings, and installs the current three bindings
(default plus alpha in `preview-alpha`, beta in `preview-beta`) in one
transaction. The preview task fails unless reconciliation then reports zero
drift. Production reconciliation keeps its report-only treatment of omitted
state unless an operator explicitly selects an authoritative namespace.
...
2. **Identity propagation and internal transport**
   ...
   - use an A/B transport keyring and keyring-revision task trigger so SSM
     rotation always rolls ECS with an overlap window.
