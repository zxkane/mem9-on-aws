...
| TC-INGEST-METRIC-032 | Synthetic blackout and boundary fixtures | Mixed-outcome windows stay quiet; an all-zero window above the volume guard breaches. Short all-zero intervals inside a mixed day verify aggregation behavior. Exactly 50 all-zero jobs stays quiet, 51 breaches, and one successful extraction above the guard clears the blackout alarm |
