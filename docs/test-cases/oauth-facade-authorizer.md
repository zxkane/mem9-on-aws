...
| TC-FACADEAUTH-004 | Synthesize the full enabled SST graph | Every workload role keeps an allowed project prefix and the exact permissions boundary; the authorizer Lambda keeps its project-prefixed name and Node 24/arm64 settings; the authorizer emits payload v2, simple responses, empty identity sources, and zero TTL; both facade routes emit `CUSTOM` authorization |
