...
| TC-EMF-006 | Emit the sampler record from the built arm64 image with the selected task-definition terminal mode | Docker captures exactly one `_aws`-first `SamplerHeartbeat=1` JSON document ending in LF without a preceding CR; its namespace, stage dimension, unit, timestamp, and content-free root shape validate |
