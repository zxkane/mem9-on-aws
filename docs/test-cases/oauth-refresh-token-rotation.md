...
| --- | --- | --- |
| TC-OAUTH-REFRESH-001 | Synthesize the reader user-pool client | Refresh-token rotation is `ENABLED` with a 10-second retry grace period, and the explicit `ALLOW_USER_SRP_AUTH` list prevents Cognito from defaulting `ALLOW_REFRESH_TOKEN_AUTH` on |
...
| TC-OAUTH-REFRESH-005 | Inspect synthesized and deployed rotation configuration | The retry grace period is exactly 10 seconds, allowing a client to retry briefly before Cognito invalidates the rotated-out token |
| TC-OAUTH-REFRESH-006 | Cognito returns a successful refresh response with a missing or empty refresh token | The facade emits a token-free diagnostic and preserves the submitted refresh token in the successful response, as required for a non-rotating RFC 6749 refresh |
...
| TC-OAUTH-REFRESH-009 | Cognito returns a successful refresh response that is not a JSON object | The facade returns a token-free `502 invalid_upstream_response` diagnostic without forwarding upstream content |
