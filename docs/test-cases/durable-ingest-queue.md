...
| TC-INGEST-METRIC-012 | Queue contains queued/retrying rows, is empty, or its age query stalls | Database-clock `OldestQueuedAgeMs` reports the oldest age or zero without tenant dimensions; a bounded asynchronous sample cannot block claims, and query failure omits only queue age |
...
| TC-INGEST-METRIC-015 | Production dashboard is synthesized | Provider and durable-application headings are separate; Mantle widgets use `AWS/BedrockMantle` plus `Project`; no Mantle latency metric appears; retry volume uses the additive stage-only `JobsRetrying` rollup, never a search gap or sum of retry ordinals; sampler heartbeat appears as telemetry health |
| TC-INGEST-METRIC-016 | Dead-job and queue-age alarms are synthesized | Dead consumes the stage-only `JobsDead` rollup and alarms at least once in 15 minutes; real queue age greater than 10 minutes alarms for two of two consecutive five-minute periods |
...
| TC-INGEST-METRIC-018 | Alarm input data is absent | Event and queue-health alarms use `notBreaching`; absent queue-age data is not proof of backlog, while absent sampler heartbeat is explicitly `breaching` |
| TC-INGEST-METRIC-019 | Production alarms are synthesized | Every action-bearing alarm targets the existing topic; raw liveness and guard alarms have no actions; application alarms use `stage=prod` and Mantle client errors use the configured `Project` |
| TC-INGEST-METRIC-020 | Legacy observability resources are synthesized | The unalarmed `ingest_dropped` metric filter is absent |
| TC-INGEST-METRIC-021 | Emit accepted, retry, success, dead, heartbeat, and queue-age EMF records | Every raw line starts with `{"_aws":`; decoding those same lines preserves the existing metadata, metric values, dimensions, units, bounded classes, and content exclusions |
| TC-INGEST-METRIC-022 | Concurrent writers emit accepted, result, heartbeat, and queue-age records to one stream | Every write produces one complete `_aws`-first JSON line; records never interleave, disappear, or acquire unbounded dimensions |
| TC-INGEST-METRIC-023 | The queue sampler starts and each minute elapses, including a queue-age query failure | Exactly one content-free `SamplerHeartbeat=1` is emitted per tick before the query, with only `stage=prod`; query failure does not suppress heartbeat |
| TC-INGEST-METRIC-024 | Sampler heartbeat is healthy, one latest sample is delayed, or five current samples are absent after older healthy samples | `FILL(SamplerHeartbeat, 0)` leaves healthy values non-breaching, tolerates one latest zero, and makes all five current misses breach without reusing older healthy datapoints |
| TC-INGEST-METRIC-025 | Liveness alarms are created without historical heartbeat samples | Five current periods fill to zero; the raw alarm has no actions, and the composite waits exactly five minutes before releasing its ALARM action if no real ECS-origin heartbeat arrives |
| TC-INGEST-METRIC-026 | A routine rollout briefly or persistently interrupts heartbeat | Fewer than five current misses remain healthy; recovery during the action wait emits no orphan OK notification, while sustained loss gets only the fixed five-minute wait |
| TC-INGEST-METRIC-027 | ECS-origin EMF disappears while `AWS/Logs` parsing and validation errors remain zero | The heartbeat absence contract alarms independently; parser error metrics and direct probes are diagnostic only |
