...
| TC-GROUPNS-062 | REST/worker startup, both maintenance CLIs, and representative DML for every scoped write table are attempted during the frozen phase | The compiled server binary refuses startup in both modes, maintenance stops before AWS/DB setup, every DML statement receives the database freeze error, and no attempted mutation changes state | Go + PostgreSQL integration |
...
| TC-GROUPNS-126 | Target and approved service issuers rotate their transport keys                                          | Stable slot `kid` values `a` and `b` verify during the overlap window; unknown issuer or `kid` fails                                                              | Unit + integration          |
...
| TC-GROUPNS-131 | ECS starts mnemo-server with the stage transport-signing key                                             | The task execution role resolves the project-scoped SSM SecureString through the existing application-region KMS boundary path; PR preview startup passes                  | Infra unit + PR preview     |
| TC-GROUPNS-132 | A PR rerun replaces one or both autogenerated A/B Cognito clients                                       | The fixed `preview-alpha`/`preview-beta` slots atomically revoke obsolete fixture access, retain exactly the current three M2M bindings, and report zero drift             | Unit + PR preview rerun     |
| TC-GROUPNS-133 | The inactive transport slot rotates, then the active slot switches in a separate deployment             | Stable slot IDs preserve both mixed-rollout overlap windows; the non-secret keyring digest changes the ECS task definition and service trigger so new tasks read the intended SSM value | Unit + infra + workflow |
| TC-GROUPNS-134 | Namespace backfill assigns ownership to historical memories and upload tasks                            | Namespace columns change without moving historical `updated_at`, preserving cleanup and ordering semantics                                                               | PostgreSQL integration      |
| TC-GROUPNS-135 | Freeze races with a writer that already holds a DML table lock                                          | Freeze waits for the writer, observes its committed non-terminal work, and rolls back without changing the migration phase                                                | PostgreSQL fault injection  |
