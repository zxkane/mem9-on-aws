...
| TC-DOCS-010 | Public-repository fork threat model | The workload-boundary design does not describe this public repository as private. It identifies the fork secret policy, fork-triggerable Infra CI `secrets.AWS_ROLE_ARN` gates, read-only fork `GITHUB_TOKEN`, and the retained `pull_request` OIDC subject without relying on repository-variable delivery. |
