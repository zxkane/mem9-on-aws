...
| TC-PITR-012 | Write fence and pre-cutover backup | Headless writers are paused, a recovery-only M2M client is the Gateway's sole trusted client, normal clients are rejected, the proxy Lambda reports zero invocations for the quiet window, and a manual snapshot of the active source cluster completes before IaC cutover |
| TC-PITR-013 | IaC cutover and synthetic verification | A reviewed IaC change updates endpoint/secret wiring, the bootstrap task updates the tenant per-request DSN, and the E2E preflight proves ECS convergence and the restored endpoint before the recovery-only client completes a synthetic write/search |
| TC-PITR-014 | Rollback | With the write fence still active, unsetting the lookup-only recovery inputs and rerunning bootstrap points both ECS and the tenant row back to the source cluster before the synthetic write/search test passes |
...
| TC-PITR-016 | Documentation CI | Runbook invariants and shell syntax pass, and public docs contain no live account IDs, ARNs, RDS endpoints, credentials, private references, or memory content |
