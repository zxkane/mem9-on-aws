...
Reference implementation: `a sibling project`'s `infra-ci.yml` + `sst.config.ts`.
...
## Verified environment facts (probed 2026-07-11, `your-aws-profile`)

Deploy profile: **`your-aws-profile`** (IAM user `kane`, account
...
│   ├── pnpm-lock.yaml
│   ├── tsconfig.json
│   ├── sst-types.d.ts         # ambient shim (a sibling project pattern)
...
`your-aws-profile`), then store the printed role ARN as the repo's
**`AWS_ROLE_ARN`** GitHub Actions secret.

## Component 5 — `.github/workflows/infra-ci.yml`

Mirrors a sibling project's `infra-ci.yml`, minus app-specific E2E. Region
...
2. Local deploy proof (`your-aws-profile`): `sst deploy --stage <dev>` →
