...
describe("unsupported maintenance deployment configuration", () => {
  it("validates the runtime budget before any deployment resources", () => {
    expect(() => assertSupportedMaintenanceConfiguration({MEM9_CONSOLIDATION_TIMEOUT_SECONDS:"7200"})).not.toThrow();
    for (const raw of ["", "59", "21601", "PRIVATE-BUDGET"]) {
      expect(() => assertSupportedMaintenanceConfiguration({MEM9_CONSOLIDATION_TIMEOUT_SECONDS:raw})).toThrow("invalid consolidation execution budget");
    }
  });
