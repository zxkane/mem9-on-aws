...
| Dimension | Decision |
|---|---|
| IaC | **SST v4**, mirroring `a sibling project` + `a sibling project` structure |
...
- `a sibling project` — the closest template. Copy from `infra/`:
...
- `a sibling project` — `sst.config.ts` shape (Tokyo region, Node 24 `$transform`,
...
- **This is a PRIVATE repo** (`zxkane/*`, kept private). It MAY reference the
  operator's private sister repos (`a sibling project`, `a sibling project`)
